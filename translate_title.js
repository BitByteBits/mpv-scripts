 ...
 * default on demand shortcut if script disabled by default: alt+t
 * to use with custom input.conf: KEY script-binding translate_title
 *
...
function translateTitle(dest) {
	...
	mp.command_native_async({
		...
		if (success) {
			...
			// don't translate if destination language is same as source but still cache to not request it again later
			if (dest == sourceLang) {
				cache[title] = title;
				saveCache(cache);
...
mp.add_key_binding("alt+t", "translate_title", function() {
	translateTitle(opts.destination);
});
